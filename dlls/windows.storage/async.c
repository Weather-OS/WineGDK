...
static inline struct async_file *impl_from_IAsyncOperation_StorageFile( IAsyncOperation_StorageFile *iface )
{
    return CONTAINING_RECORD( iface, struct async_file, IAsyncOperation_StorageFile_iface );
}

static HRESULT WINAPI async_file_QueryInterface( IAsyncOperation_StorageFile *iface, REFIID iid, void **out )
{
    struct async_file *impl = impl_from_IAsyncOperation_StorageFile( iface );
    ...
    {
        IInspectable_AddRef( (*out = &impl->IAsyncOperation_StorageFile_iface) );
        return S_OK;
    }
...
static ULONG WINAPI async_file_AddRef( IAsyncOperation_StorageFile *iface )
{
    struct async_file *impl = impl_from_IAsyncOperation_StorageFile( iface );
    ULONG ref = InterlockedIncrement( &impl->ref );
    TRACE( "iface %p, ref %lu.\n", iface, ref );
...
static ULONG WINAPI async_file_Release( IAsyncOperation_StorageFile *iface )
{
    struct async_file *impl = impl_from_IAsyncOperation_StorageFile( iface );
    ULONG ref = InterlockedDecrement( &impl->ref );
    TRACE( "iface %p, ref %lu.\n", iface, ref );
...
static HRESULT WINAPI async_file_GetIids( IAsyncOperation_StorageFile *iface, ULONG *iid_count, IID **iids )
{
    FIXME( "iface %p, iid_count %p, iids %p stub!\n", iface, iid_count, iids );
    return E_NOTIMPL;
}

static HRESULT WINAPI async_file_GetRuntimeClassName( IAsyncOperation_StorageFile *iface, HSTRING *class_name )
{
    FIXME( "iface %p, class_name %p stub!\n", iface, class_name );
    return E_NOTIMPL;
}

static HRESULT WINAPI async_file_GetTrustLevel( IAsyncOperation_StorageFile *iface, TrustLevel *trust_level )
{
    FIXME( "iface %p, trust_level %p stub!\n", iface, trust_level );
    return E_NOTIMPL;
}

static HRESULT WINAPI async_file_put_Completed( IAsyncOperation_StorageFile *iface, IAsyncOperationCompletedHandler_StorageFile *file_handler )
{
    IAsyncOperationCompletedHandlerImpl *handler = (IAsyncOperationCompletedHandlerImpl *)file_handler;
    struct async_file *impl = impl_from_IAsyncOperation_StorageFile( iface );
    TRACE( "iface %p, handler %p.\n", iface, handler );
    return IAsyncInfoImpl_put_Completed( impl->IAsyncInfoImpl_inner, handler );
}

static HRESULT WINAPI async_file_get_Completed( IAsyncOperation_StorageFile *iface, IAsyncOperationCompletedHandler_StorageFile **file_handler )
{
    IAsyncOperationCompletedHandlerImpl **handler = (IAsyncOperationCompletedHandlerImpl **)file_handler;
    struct async_file *impl = impl_from_IAsyncOperation_StorageFile( iface );
    TRACE( "iface %p, handler %p.\n", iface, handler );
...
static HRESULT WINAPI async_file_GetResults( IAsyncOperation_StorageFile *iface, IStorageFile **results )
{
    struct async_file *impl = impl_from_IAsyncOperation_StorageFile( iface );
    ...
    *results = (IStorageFile *)result.punkVal;
    PropVariantClear( &result );
    return hr;
}

static const struct IAsyncOperation_StorageFileVtbl async_file_vtbl =
{
    /* IUnknown methods */
    async_file_QueryInterface,
    async_file_AddRef,
    async_file_Release,
    /* IInspectable methods */
    async_file_GetIids,
    async_file_GetRuntimeClassName,
    async_file_GetTrustLevel,
    /* IAsyncOperation<StorageFile> */
    async_file_put_Completed,
    async_file_get_Completed,
    async_file_GetResults,
};

HRESULT async_operation_file_create( IUnknown *invoker, IUnknown *param, async_operation_callback callback,
                                        IAsyncOperation_StorageFile **out )
{
    struct async_file *impl;
    HRESULT hr;

    *out = NULL;
    if (!(impl = calloc( 1, sizeof(*impl) ))) return E_OUTOFMEMORY;
    impl->IAsyncOperation_StorageFile_iface.lpVtbl = &async_file_vtbl;
    impl->ref = 1;

    if (FAILED(hr = async_info_create( invoker, param, callback, (IInspectable *)&impl->IAsyncOperation_StorageFile_iface, &impl->IAsyncInfoImpl_inner )) ||
    ...
    *out = &impl->IAsyncOperation_StorageFile_iface;
