...
{
    static const WCHAR *file_save_picker_name = L"Windows.Storage.Pickers.FileSavePicker";
    IFileOpenPicker *file_save_picker = (void *)0xdeadbeef;
    IActivationFactory  *factory = (void *)0xdeadbeef;
    HSTRING str;
    HRESULT hr;
    LONG ref;

    hr = WindowsCreateString( file_save_picker_name, wcslen( file_save_picker_name ), &str );
    ok( hr == S_OK, "got hr %#lx.\n", hr );
    hr = RoGetActivationFactory( str, &IID_IActivationFactory, (void **)&factory );
    WindowsDeleteString( str );
    ok( hr == S_OK || broken( hr == REGDB_E_CLASSNOTREG ), "got hr %#lx.\n", hr );
    if (hr == REGDB_E_CLASSNOTREG)
    {
        win_skip( "%s runtimeclass not registered, skipping tests.\n", wine_dbgstr_w( file_save_picker_name ) );
        return;
    }
    ...
    hr = IActivationFactory_ActivateInstance( factory, (IInspectable **)&file_save_picker );
    ok( hr == S_OK, "got hr %#lx.\n", hr );

    check_interface( file_save_picker, &IID_IUnknown, FALSE );
    check_interface( file_save_picker, &IID_IInspectable, FALSE );
    check_interface( file_save_picker, &IID_IFileSavePicker, FALSE );
    check_interface( file_save_picker, &IID_IInitializeWithWindow, FALSE );

    ref = IFileOpenPicker_Release( file_save_picker );
    ok( ref == 0, "got ref %ld.\n", ref );
    ref = IActivationFactory_Release( factory );
    ok( ref == 0, "got ref %ld.\n", ref );
}
