...
HRESULT json_array_push( IJsonArray *iface, IJsonValue *value )
{
    ...
    if (impl->length == impl->size)
        new = realloc(new, ++impl->size * sizeof(IJsonValue*));
...
HRESULT json_array_pop( IJsonArray *iface, IJsonValue **value )
{
    struct json_array *impl = impl_from_IJsonArray( iface );

    TRACE( "iface %p, value %p.\n", iface, value );

    if (impl->length == 0) return E_FAIL;

    if (value) *value = impl->elements[--impl->length];
    else IJsonValue_Release( impl->elements[--impl->length] );
    return S_OK;
}
